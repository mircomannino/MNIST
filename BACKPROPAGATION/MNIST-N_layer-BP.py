        ...
        for epoch in range(n_epochs):
            ...
            for k in range(nExamples):
                ...
                for i in range(self.outputLayer.dim()):
                    ...
                    grad = delta_error * x
                    self.outputLayer.neurons[i].weight -= learning_rate * grad
                delta_errors.append(delta_error_out)
                ###############################print(delta_error_out)
                ### Hidden layer
                ...
                for inv_j in range(len(self.hiddenLayers)):
                    ...
                    for i in range(self.hiddenLayers[j-1].dim()):
                        ...
                        for c in range(10):
                            delta_error_children = delta_errors[d_counter][i]
                            h_weight = 0
                            if(inv_j == 0):
                                h_weight = self.outputLayer.neurons[i].weight
                            else:
                                h_weight = self.hiddenLayers[j].neurons[i].weight
                ...
                for i in range(self.firstLayer.dim()):
                    ...
                    for c in range(10):
                        delta_error_children = delta_errors[d_counter][i]
                        h_weight = self.hiddenLayers[0].neurons[i].weight
                        sum += h_weight * delta_error_children
                    delta_error = x * sum
                    grad = delta_error * x
